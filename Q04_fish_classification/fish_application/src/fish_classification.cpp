...
void show_result(int out)
{
    if (out != -1)
    {
        ...
        cv::putText(frame,  "Fish class: " + std::string(class_names[out]), cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 1.0, GREEN, 2);
    }
    else
        cv::putText(frame,  "Cannot Identify !", cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 1.0, RED, 2);
}

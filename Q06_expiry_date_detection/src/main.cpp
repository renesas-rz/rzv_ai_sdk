...
{
    std::lock_guard<std::mutex> lck {date_mtx};
...
cv::Mat print_result(cv::Mat &frame)
{
    ...
    for (int i = 0; i < det.size(); i++)
    {
        ...
        if (det[i].c == 0 && (date_struc_map.find(i) != date_struc_map.end()))
        {
            ...
            if (rem_days_shown)
            {
                ...
                {
                    cv::putText(frame, "Remaining Days: " + std::to_string(date_struc_map.at(i).remaining_days), cv::Point(x+10, 570 + 120*date_cnt), 
    ...
    cv::putText(frame, "Total AI time(ms)  : " + float_to_string(total_time), cv::Point(x+20, 60), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    cv::putText(frame, " Inference time    : " + float_to_string(ai_time), cv::Point(x+20, 90), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    cv::putText(frame, " PreProcess  time : " + float_to_string(pre_time), cv::Point(x+20, 120), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    cv::putText(frame, " Postprocess time : " + float_to_string(post_time), cv::Point(x+20, 150), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);

...
static std::unordered_map<int, date_struct> date_struc_map;
static std::unordered_map<int, date_struct> memory_date_struc_map;
/*Variable for Regex Dict */
static std::map<boost::regex, std::string> regex_dict_g;

cv::Mat yuyv_image;
cv::Mat input_image;
cv::Mat frame_g;

std::string ymd_string = "";
...
int32_t yolo_offset(uint8_t n, int32_t b, int32_t y, int32_t x)
{
    ...
    for (i = 0 ; i < n; i++)
    {
        prev_layer_num += NUM_BB *(NUM_CLASSES + 5)* num_grids[i] * num_grids[i];
    }
    return prev_layer_num + b *(NUM_CLASSES + 5)* num * num + y * num + x;
}
        ...
        for(b = 0; b < NUM_BB; b++)        
        {
            for(y = 0; y < num_grid; y++)
            {
                for(x = 0; x < num_grid; x++)
                {
                    ...
                    Boxb bb = {center_x, center_y, box_w, box_h};
...
void date_extraction(void)
{
    ...
    for (int i = 0; i < det.size(); i++)
    {
        ...
        std::cout<< "Detected String :"<< processed_text << std::endl;
        ...
        if (processed_text.length() >= 1)
        {
            ...
            {
                std::cout << "The string '" << processed_text << "' match the format " << result_struc.format<< std::endl;
                ...
                std::cout << "Year : " << result_struc.year << " Month : " << result_struc.month << " Day: " << result_struc.day<<std::endl;
                ...
                if (rem_days_shown)
                {
                    ...
                    std::cout<< "Days Remaining "<< day_rem <<std::endl;
...
void *R_Capture_Thread(void *cap_pipeline)
{
    ...
    if (g_imagemode)
    {
      /* Breaking the loop if no video frame is detected */
        if (g_imgframe.empty()&&g_imagemode)
         {
...
cv::Mat print_result(cv::Mat &frame)
{
    ...
    cv::Point text_pos(1540, 1020);
    std::stringstream stream;
    std::string str = "";
    int32_t result_cnt = 0;
    int32_t date_cnt = 0;
    int32_t date_cnt2 = 0;
    uint32_t x = DISP_IMAGE_OUTPUT_WIDTH + 20;
    uint32_t y = 175;
    ...
    for (int i = 0; i < det.size(); i++)
    {
        ...
        label_file_map[det[i].c].erase(std::remove_if(label_file_map[det[i].c].begin(), label_file_map[det[i].c].end(),
         ::isspace), label_file_map[det[i].c].end());
        stream << "Class : " << label_file_map[det[i].c].c_str() << " " << round(det[i].prob * 100) << "%";
        str = stream.str();
        y = LINE_HEIGHT * result_cnt;
        cv::putText(frame, str,cv::Point(x+20, 175 + result_cnt*25), CV_FONT, CHAR_SCALE_SMALL, CV_WHITE, CHAR_THICKNESS_OUT);
        ...
        if (det[i].c == 0 && (date_struc_map.find(i) != date_struc_map.end()))
        {
            ...
            memory_date_struc_map[i].day = date_struc_map[i].day;
            memory_date_struc_map[i].month = date_struc_map[i].month;
            memory_date_struc_map[i].year = date_struc_map[i].year;
            cv::putText(frame, "Day : " + date_struc_map[i].day, cv::Point(x+10, 480 + 120*date_cnt), 
                        CV_FONT, CHAR_SCALE_LARGE, CV_WHITE, CHAR_THICKNESS_OUT);
            cv::putText(frame, "Month : " + date_struc_map[i].month, cv::Point(x+10, 510 + 120*date_cnt), 
                        CV_FONT, CHAR_SCALE_LARGE, CV_WHITE, CHAR_THICKNESS_OUT);
            cv::putText(frame, "Year : " + date_struc_map[i].year, cv::Point(x+10, 540 + 120*date_cnt), 
                        CV_FONT, CHAR_SCALE_LARGE, CV_WHITE, CHAR_THICKNESS_OUT);
            if (rem_days_shown)
            {
                memory_date_struc_map[i].remaining_days = date_struc_map[i].remaining_days;
                if (date_struc_map[i].remaining_days == -1)
                {
                    cv::putText(frame, "Date Expired !!", cv::Point(x+10, 570 + 120*date_cnt), 
                                CV_FONT, CHAR_SCALE_LARGE, CV_RED, CHAR_THICKNESS_OUT);
                }
                else
                {
                    cv::putText(frame, "Remaining Days: " + date_struc_map[i].remaining_days, cv::Point(x+10, 570 + 120*date_cnt), 
                                CV_FONT, CHAR_SCALE_LARGE, CV_GREEN, CHAR_THICKNESS_OUT);
                }
            }
            date_cnt++;
        } 
        else if(det[i].c == 0 && (memory_date_struc_map.find(i) != memory_date_struc_map.end()))
        {
            
            cv::putText(frame, "Day : " + memory_date_struc_map[i].day, cv::Point(x+10, 480 + 120*date_cnt2), 
                        CV_FONT, CHAR_SCALE_LARGE, CV_WHITE, CHAR_THICKNESS_OUT);
            cv::putText(frame, "Month : " + memory_date_struc_map[i].month, cv::Point(x+10, 510 + 120*date_cnt2), 
                        CV_FONT, CHAR_SCALE_LARGE, CV_WHITE, CHAR_THICKNESS_OUT);
            cv::putText(frame, "Year : " + memory_date_struc_map[i].year, cv::Point(x+10, 540 + 120*date_cnt2), 
                        CV_FONT, CHAR_SCALE_LARGE, CV_WHITE, CHAR_THICKNESS_OUT);
            if (rem_days_shown)
            {
                if (memory_date_struc_map[i].remaining_days == -1)
                {
                    cv::putText(frame, "Date Expired !!", cv::Point(x+10, 570 + 120*date_cnt2), 
                                CV_FONT, CHAR_SCALE_LARGE, CV_RED, CHAR_THICKNESS_OUT);
                }
                else
                {
                    cv::putText(frame, "Remaining Days: " + memory_date_struc_map[i].remaining_days, cv::Point(x+10, 570 + 120*date_cnt2), 
                                CV_FONT, CHAR_SCALE_LARGE, CV_GREEN, CHAR_THICKNESS_OUT);
                }
            }
            date_cnt2++;
    ...
    float font_weight       = 2;
    float font_size         = 0.87;
    cv::putText(frame, "Total AI time(ms)  : " + std::to_string(int(total_time)), cv::Point(x+20, 60), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    cv::putText(frame, " Inference time    : " + std::to_string(int(ai_time)), cv::Point(x+20, 90), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    cv::putText(frame, " PreProcess  time : " + std::to_string(int(pre_time)), cv::Point(x+20, 120), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    cv::putText(frame, " Postprocess time : " + std::to_string(int(post_time)), cv::Point(x+20, 150), 
                cv::FONT_HERSHEY_DUPLEX, font_size, cv::Scalar(255, 255, 255), font_weight);
    // cv::putText(frame, text, text_pos, 
 ...
 while(1)
    {
        ...
        if (img_obj_ready.load())
        {
            ...
            bgra_image = draw_bounding_box(bgra_image);
            bgra_image = create_output_frame(bgra_image);
            bgra_image = print_result(bgra_image);       
            mtx.unlock();
...
int32_t main(int32_t argc, char * argv[])
{
    ...
    if (args.find("--rem") != args.end())
    {
     if (args["--rem"]=="true")
        rem_days_shown = true;
     else if(args["--rem"]=="false")
        rem_days_shown = false;
     else
        errorHandle=true;
    }
    ...
    else 
    {
      ...
      if (input_source == "USB" || input_source == "IMAGE" && argc >= 2)
        errorHandle = false;
      else
        errorHandle = true;
    ...
    switch (input_source_map[input_source])
    {
        ...
        case 2:
        {
            ...
            /* extract the file extension of image path */
            size_t dotPos = image_path.find_last_of('.');
            std::string extension = image_path.substr(dotPos + 1);
            if (dotPos != std::string::npos && extension == "jpg" || extension == "JPG" || extension =="png"|| 
            extension =="PNG" || extension == "jpeg" || extension == "JPEG") 
    ...
    INF_OUT_SIZE = (NUM_CLASSES + 5) * NUM_BB * num_grids[0] * num_grids[0] 
                    + (NUM_CLASSES + 5) * NUM_BB * num_grids[1] * num_grids[1]
                    + (NUM_CLASSES + 5) * NUM_BB * num_grids[2] * num_grids[2];

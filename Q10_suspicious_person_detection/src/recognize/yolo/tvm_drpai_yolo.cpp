 ...
 * @param id Mode
 */
TVM_YOLO_DRPAI::TVM_YOLO_DRPAI(uint8_t id) : IRecognizeModel(0, TVM_MODEL_DIR_YOLOV3.data(), 
...
{
    int8_t ret = 0;
    ini_values = CommonFunc::config_read("config.ini");
    /* Load Preprocess Directory*/
    ret = preruntime.Load(ini_values["path"]["model_path"]+"/preprocess");
    if (ret < 0)
    {
    ...
    label_file_map = CommonFunc::load_label_file(ini_values["path"]["label_path"]);

    conf = std::stof(ini_values["detect"]["conf"]);
    get_anchor = ini_values["detect"]["anchors"];
    detection_object_string = ini_values["detect"]["objects"];

...
3. Go to the application source code directory.  
    ```sh
    cd ${PROJECT_PATH}/Q10_suspicious_person_detection/<SRC_DIR>
    ```
    |Board | `SRC_DIR` |
    |:---|:---|
    |RZ/V2L EVK|`src`  |
...
5. Build the application by following the commands below.  
    ...
    make -j$(nproc)
    ```
6. The following application file would be generated in the `${PROJECT_PATH}/Q10_suspicious_person_detection/<SRC_DIR>/build` directory
...
1. [FOR RZ/V2H only] Run following commands to download the necessary file. 
   ```sh
   cd <path_to_data_folder_on_host>/rzv_ai_sdk/Q10_suspicious_person_detection/exe_v2h/suspicious_onnx
   wget https://github.com/renesas-rz/rzv_ai_sdk/releases/download/v4.00/Q10_suspicious_person_detection_deploy_tvm_v2h-v221.so
   ```

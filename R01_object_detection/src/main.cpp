...
void *R_Inf_Thread(void *threadid)
{
    ...
    while(1)
    {
        ...
        double total_time = ai_time + pre_time + post_time;
        printf("Total AI Time: %lf[ms]\n", std::round(total_time * 10) / 10);
        printf("Inference: %lf[ms]\n", std::round(ai_time * 10) / 10);
        printf("PreProcess: %lf[ms]\n", std::round(pre_time * 10) / 10);
        printf("PostProcess: %lf[ms]\n", std::round(post_time * 10) / 10);
...
int8_t R_Main_Process()
{
    ...
    while(1)
    {
        ...
        if (img_obj_ready.load())
        {
            ...
            main_proc_time = (timedifference_msec(start_time, end_time) * TIME_COEF);
            
            printf("Main Proc Time       : %lf[ms]\n", main_proc_time);
            #endif  /* DEBUG_TIME_FLG */
        }
        #endif  /* for V2L */
